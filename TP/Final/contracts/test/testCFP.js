...
describe("Call for Proposals", function () {
  ...
  describe("Entrega de archivos post-cierre", function () {
    let cfp;
    let proposalHash;
    let closingTime;
    ...
    before(async function () {
        ...
        await cfp.registerProposal(proposalHash);

        // Avanzar el tiempo para cerrar la convocatoria
        await networkHelpers.time.increaseTo(closingTime + 1);
    });

    it("debe permitir registrar una entrega después del cierre", async () => {
        const filesRoot = gen.next();
        await cfp.registerDelivery(proposalHash, filesRoot);
        const delivery = await cfp.deliveryData(proposalHash);
        expect(delivery.delivered).to.be.true;
        expect(delivery.filesRoot).to.equal(filesRoot);
        expect(delivery.sender).to.equal(accounts[0].address);
        expect(delivery.blockNumber).to.be.gt(0n);
        expect(delivery.timestamp).to.be.gt(0n);
    });

    it("debe emitir el evento FilesDelivered al registrar una entrega", async () => {
        const CFP = await ethers.getContractFactory("CFP");
        const latestBlock = await ethers.provider.getBlock("latest");
        ...
        const filesRoot = gen.next();
        await expect(newCfp.registerDelivery(proposal, filesRoot))
            .to.emit(newCfp, "FilesDelivered")
            .withArgs(proposal, filesRoot, accounts[0].address, await ethers.provider.getBlock("latest").then(b => BigInt(b.timestamp + 1)));
    });

    it("debe rechazar la entrega si la convocatoria no ha cerrado", async () => {
        const CFP = await ethers.getContractFactory("CFP");
        const latestBlock = await ethers.provider.getBlock("latest");
        ...
        await newCfp.registerProposal(proposal);
        await expect(
            newCfp.registerDelivery(proposal, gen.next())
        ).to.be.revertedWith("La convocatoria no ha cerrado");
    });

    it("debe rechazar la entrega si la propuesta no existe", async () => {
        const propuestaInexistente = gen.next();
        await expect(
            cfp.registerDelivery(propuestaInexistente, gen.next())
        ).to.be.revertedWith("La propuesta no existe");
    });

    it("debe rechazar la entrega duplicada", async () => {
        await expect(
            cfp.registerDelivery(proposalHash, gen.next())
        ).to.be.revertedWith("La entrega ya fue registrada");
    });

    it("debe devolver datos vacíos para una propuesta sin entrega", async () => {
        // Crear un CFP nuevo que ya esté cerrado
        const latestBlock = await ethers.provider.getBlock("latest");
        const ct = latestBlock.timestamp + 5;
        const CFP = await ethers.getContractFactory("CFP");
        const newCfp = await CFP.deploy(gen.next(), BigInt(ct), 0n, ethers.ZeroAddress, accounts[0].address);
        ...
        await newCfp.registerProposal(propuestaSinEntrega);

        // Avanzar el tiempo para cerrar
        await networkHelpers.time.increaseTo(ct + 1);

        // Consultar entrega sin haberla registrado
        const delivery = await newCfp.deliveryData(propuestaSinEntrega);
        expect(delivery.delivered).to.be.false;
        expect(delivery.blockNumber).to.equal(0n);
    });
  });
});
