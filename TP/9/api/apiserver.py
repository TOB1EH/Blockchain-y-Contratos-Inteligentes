...
def recover_address(message_bytes: bytes, signature: str) -> str:
    """
    Recupera la direccion del firmante.
    Lanza ValueError si la firma tiene formato inválido.
    """

    # Asegurar que la firma tenga el prefijo '0x' para evitar problemas al convertirla a bytes
...
def calls(call_id):
    ...
    try:
        # Intentar obtener la informacion del llamado desde la base de datos. Si no existe, devolver error 404.
...
def patch_registration(address):
    ...
    try:
        ...
        # Mensaje: UPDATE_PREFIX + contract(20) + nonce(32) + name
        try:
            message = (
                UPDATE_PREFIX
                + bytes.fromhex(FACTORY_ADDRESS[2:])
...
def authorize(address):
    ...
    # Validar que la firma sea del administrador y que el nonce sea correcto para evitar
    # que alguien pueda reutilizar una firma valida para autorizar direcciones diferentes
    # o para autorizar la misma direccion varias
    try:
